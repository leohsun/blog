import React from 'react'
import Publish from './publish'
import EditArticle from './edit-article'
import { Link, Route } from 'react-router-dom'
import {inject,observer} from 'mobx-react'
import { Layout, Menu, Icon,Spin } from 'antd'
const { SubMenu } = Menu;
const { Header, Sider, Content } = Layout


// page
// import Editor from './editor'

const Analysis = () => <h1>analysis</h1>


const RouterView = () => {
  return (
    <div>
      <Route path="/admin/analysis" component={Analysis}></Route>
      <Route path="/admin/publish" component={Publish}></Route>
      <Route path="/admin/analysis" component={Analysis}></Route>
      <Route path="/admin/edit-article" component={EditArticle}></Route>
    </div>
  )
}


@inject('adminStore')
@observer
export default class Home extends React.Component {
  state={
    collapsed:false
  }
  toggle = () => {
    this.setState({
      collapsed: !this.state.collapsed,
    });
  }
  componentDidMount(){
    console.log(this.props);
  }

  render() {
    return (
      <Spin spinning={this.props.adminStore.loading} wrapperClassName="layout-wrapper">
        <Layout style={{ height: "100%" }} className="layout">
          <Sider
            trigger={null}
            collapsible
            collapsed={this.state.collapsed}
          >
            <div className="logo" />
            <Menu theme="dark" mode="inline" defaultSelectedKeys={['1']}>
              <Menu.Item key="1">
                <Icon type="dot-chart" />
                <Link to="/admin/analysis">网站统计</Link>
              </Menu.Item>
              <SubMenu
                key="sub1"
                title={<span><Icon type="code" /><span>内容管理</span></span>}
              >
                <Menu.Item key="3">
                  <Link to="/admin/publish">发表文章</Link>
                </Menu.Item>
                <Menu.Item key="4"><Link to="/admin/edit-article">管理文章</Link></Menu.Item>
              </SubMenu>

            </Menu>
          </Sider>
          <Layout>
            <Header style={{ background: '#fff', paddingLeft: 0, fontSize: '30px' }}>
              <Icon
                className="trigger"
                type={this.state.collapsed ? 'menu-unfold' : 'menu-fold'}
                onClick={this.toggle}
              />
            </Header>
            <Content style={{ margin: '24px 16px', padding: 24, background: '#fff', minHeight: 'ß' }}>
              <RouterView></RouterView>
            </Content>
          </Layout>
        </Layout>
      </Spin>
    )
  }
}