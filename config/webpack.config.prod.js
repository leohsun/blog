...
const deployDate = () => {
	...
	return `${year}${symbol}${month > 9 ? month : '0' + month}${symbol}${day > 9 ? day : '0' + day}-${hours > 9
		? hours
...
module.exports = {
	...
	plugins: [
		...
		new webpack.optimize.CommonsChunkPlugin({
			...
			chunks:['vendor'],
			minChunks: function(module){
				return module.resource && (/antd/).test(module.resource);
			}
		}),
		new webpack.optimize.CommonsChunkPlugin({
			name: 'react',
		...
		new webpack.optimize.CommonsChunkPlugin({
			...
			chunks:['vendor'],
			minChunks: function(module){
				return module.resource && (/axios|qs|lodash|mobx/).test(module.resource);
			}
		}),
		...
		new HtmlWebpackPlugin({
			...
			chunksSortMode: function(a, b) {
				console.log(a.names)
				var order = ['manifest','tools','react','antd-ui','vendor','main'];
				var order1 = order.indexOf(a.names[0]);
				var order2 = order.indexOf(b.names[0]);
